...
public class ProductDetail_Vx0 {

    public static ProductDetailOutput process(ProductDetailInput input) throws SQLException {
    ...
    public static ProductDetailOutput getData(Connection conn, ProductDetailInput input) throws SQLException {
        List<Integer> list = input.getItemIds();
        ProductDetailOutput pdo = new ProductDetailOutput();
        String sql = "SELECT i_id, i_title, a_fname, a_lname, i_pub_date, i_publisher, i_subject, " +
                ...
                "      WHERE a_id = i_a_id" +
                "        AND i_id = ";
        PreparedStatement ps64 = conn.prepareStatement(sql);
        if (list != null) {
            int tam = list.size();
    ...
    public ProductDetailOutput getHardCodeData(ProductDetailInput input) throws SQLException {
        ProductDetailOutput pdo = new ProductDetailOutput();
        ProductDetailPojo pojo = new ProductDetailPojo();
        pojo.setItemId(11111111L);
        if (pojo.getItemId().equals(input.getId())) {
            pojo.setItemId(000000001L);
            pojo.setItemTitle("Pride and Prejudice");
            pojo.setAuthorFName("Jane");
            pojo.setAuthorLName("Austen");
            pdo.getDetailedProductsList().add(pojo);
        }
        return (pdo);
    }
}